...
<head>
    ...
    <!-- Bootstrap core CSS -->
    <link rel="stylesheet" href="{{ asset('css/bootstrap.min.css') }}" integrity="sha384-r4NyP46KrjDleawBgD5tp8Y7UzmLA05oM1iAEQ17CSuDqnUK2+k9luXQOfXJCJ4I" crossorigin="anonymous">
...
<div class="container">
    <x-header/>
    @if (isset($categoryId))
        <x-category_menu :categories="$categories" :categoryId="$categoryId" />
    @else
        <x-category_menu :categories="$categories" />
    @endif
    @isset($isHomeIndexPage)
        <x-home_index_header :pageTitle="$pageTitle"/>
    @endisset
</div>
<main class="container">
    @if (!isset($isHomeIndexPage))
        <x-page_title :pageTitle="$pageTitle"/>
    @endif
    @yield('content')
</main>
<x-footer/>
